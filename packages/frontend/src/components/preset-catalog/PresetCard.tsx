...
export function PresetCard({ preset, onSelect }: PresetCardProps) {
  ...
  return (
    ...
    >
      {/* 상단: 큰 썸네일 */}
      <div className="mb-2.5 rounded-md bg-gray-900/60 p-2" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 100 }}>
        <PresetThumbnail
          category={category}
          code={preset.code ?? ''}
          width={preset.width}
      ...
      <div className="mt-1.5 flex flex-wrap gap-1">
        <span className="rounded bg-gray-700 px-1.5 py-0.5 text-[10px] font-mono text-gray-300 truncate max-w-[120px]">
          {preset.code}
        </span>
        {preset.region && (
