...
> 모든 작업 완료 시 아래 항목을 반드시 사용자에게 보고한다.

1. **새로 추가된 npm 패키지** — 패키지명, 용도, 설치 명령어 (`npm install` 또는 `yarn add`)
2. **DB 스키마 변경** — 변경된 테이블/컬럼, 마이그레이션 실행 명령어 (`npx prisma migrate dev` 등)
3. 변경 없는 경우에도 "변경 없음"을 명시적으로 알려줄 것
